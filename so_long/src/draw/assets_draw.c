...
/*   Updated: 2025/01/20 23:29:28 by honnguye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_set_map_img(t_graphics **graphics, t_map *map)
{
	...
	(*graphics)->wall = ft_draw_asset(graphics, (*graphics)->mlx, water,
			map->walls);
	(*graphics)->space = ft_draw_asset(graphics, (*graphics)->mlx, grass,
			map->spaces);
	(*graphics)->collectable = ft_draw_asset(graphics, (*graphics)->mlx,
			collectable, map->collectibles);
	(*graphics)->exit = ft_draw_exit(graphics, (*graphics)->mlx, tower,
			map->exit);
	(*graphics)->enemy = ft_draw_asset(graphics, (*graphics)->mlx, enemy,
			(*graphics)->map->enemy);
